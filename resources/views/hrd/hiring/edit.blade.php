...
@section('dashboard-content')
    @php
        ...
        $genderStatus = old('req_gender_status', $config['gender']['status'] ?? 'core');
        $genderValue = old('req_gender_value', $config['gender']['value'] ?? 'male');
        $ageStatus = old('req_age_status', $config['age']['status'] ?? 'core');
        $ageMin = old('req_age_min', $config['age']['min'] ?? 25);
        $ageMax = old('req_age_max', $config['age']['max'] ?? 35);
        $educationStatus = old('req_education_status', $config['education']['status'] ?? 'core');
        $educationValue = old('req_education_value', $config['education']['value'] ?? 'SMA/SMK');
        $agdStatus = old('req_agd_status', $config['agd']['status'] ?? 'secondary');
        $simcStatus = old('req_sim_c_status', $config['sim_c']['status'] ?? 'secondary');
        $simb1Status = old('req_sim_b1_status', $config['sim_b1']['status'] ?? 'secondary');
        $experienceStatus = old('req_experience_status', $config['experience']['status'] ?? 'secondary');
        $experienceValue = old('req_experience_value', $config['experience']['value'] ?? 0);
        $placementStatus = old('req_placement_ready_status', $config['placement_ready']['status'] ?? 'core');
        $placementType = old('req_placement_type', $config['placement_ready']['type'] ?? 'anywhere');
        $salaryHidden = old('salary_hidden', $posting->salary_hidden) ? 'true' : 'false';

        // Custom requirements
        $majorStatus = old('req_major_status', $config['major']['status'] ?? 'nonaktif');
        $majorValue = old('req_major_value', $config['major']['value'] ?? 'Keperawatan, Asisten Keperawatan');
        $placementChoicesStatus = old('req_placement_choices_status', $config['placement_choices']['status'] ?? 'nonaktif');
        $placementChoicesValue = old('req_placement_choices_value', $config['placement_choices']['value'] ?? 'Cakung (Jakarta Timur), Lebak Bulus (Jakarta Selatan)');
        $medicalSupportStatus = old('req_medical_support_status', $config['medical_support']['status'] ?? 'nonaktif');
        $medicalTermsStatus = old('req_medical_terms_status', $config['medical_terms']['status'] ?? 'nonaktif');
        $gardenerTechStatus = old('req_gardener_tech_understanding_status', $config['gardener_tech_understanding']['status'] ?? 'nonaktif');
        $gardenerNurseryStatus = old('req_gardener_nursery_skill_status', $config['gardener_nursery_skill']['status'] ?? 'nonaktif');
        $gardenerToolsStatus = old('req_gardener_tools_skill_status', $config['gardener_tools_skill']['status'] ?? 'nonaktif');
        $customDocs = old('custom_documents', $config['custom_documents'] ?? []);
    @endphp
    ...
    <div class="space-y-6 animate-fade-in">
        <div class="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden">
            ...
            <form action="{{ route('hrd.hiring.update', $posting) }}" method="POST" class="space-y-8" x-data="{
                category: '{{ old('category', $posting->category) }}',
                genderStatus: '{{ $genderStatus }}',
                ageStatus: '{{ $ageStatus }}',
                educationStatus: '{{ $educationStatus }}',
                agdStatus: '{{ $agdStatus }}',
                simcStatus: '{{ $simcStatus }}',
                simb1Status: '{{ $simb1Status }}',
                experienceStatus: '{{ $experienceStatus }}',
                placementStatus: '{{ $placementStatus }}',
                placementType: '{{ $placementType }}',
                majorStatus: '{{ $majorStatus }}',
                placementChoicesStatus: '{{ $placementChoicesStatus }}',
                medicalSupportStatus: '{{ $medicalSupportStatus }}',
                medicalTermsStatus: '{{ $medicalTermsStatus }}',
                gardenerTechStatus: '{{ $gardenerTechStatus }}',
                gardenerNurseryStatus: '{{ $gardenerNurseryStatus }}',
                gardenerToolsStatus: '{{ $gardenerToolsStatus }}',
                salaryHidden: {{ $salaryHidden }},
                customDocs: {{ json_encode($customDocs) }},
                slugify(text) {
                    if (!text) return '';
                    return text
                        .toString()
                        ...
                        <div>
                            ...
                            <select name="category" id="category-select" x-model="category"
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category }}" @selected(old('category', $posting->category) === $category)>
                                        {{ $category }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="req_gender_value" :disabled="genderStatus === 'nonaktif'" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-blue-500 transition-all" :class="genderStatus === 'nonaktif' ? 'bg-slate-50 text-slate-400 border-slate-100' : 'bg-white text-slate-800'">
                                <option value="male" @selected($genderValue === 'male')>Pria saja</option>
                                <option value="female" @selected($genderValue === 'female')>Wanita saja</option>
                                <option value="both" @selected($genderValue === 'both')>Pria dan Wanita</option>
                            </select>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="flex items-center gap-1 bg-white rounded-xl border border-slate-200 px-3 py-2 text-xs focus-within:border-blue-500 transition-all" :class="ageStatus === 'nonaktif' ? 'bg-slate-50 text-slate-400 border-slate-105 shadow-none' : 'bg-white'">
                                    ...
                                    <input type="number" name="req_age_min" :disabled="ageStatus === 'nonaktif'" value="{{ $ageMin }}" min="18" max="60" class="w-full bg-transparent focus:outline-none font-bold" :class="ageStatus === 'nonaktif' ? 'text-slate-400' : 'text-slate-800'">
                                </div>
                                <div class="flex items-center gap-1 bg-white rounded-xl border border-slate-200 px-3 py-2 text-xs focus-within:border-blue-500 transition-all" :class="ageStatus === 'nonaktif' ? 'bg-slate-50 text-slate-400 border-slate-105 shadow-none' : 'bg-white'">
                                    <span class="text-slate-400">Max:</span>
                                    <input type="number" name="req_age_max" :disabled="ageStatus === 'nonaktif'" value="{{ $ageMax }}" min="18" max="65" class="w-full bg-transparent focus:outline-none font-bold" :class="ageStatus === 'nonaktif' ? 'text-slate-400' : 'text-slate-800'">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" name="req_experience_value" :disabled="experienceStatus === 'nonaktif'" value="{{ $experienceValue }}" min="0" max="50" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-blue-500 transition-all" :class="experienceStatus === 'nonaktif' ? 'bg-slate-50 text-slate-400 border-slate-100' : 'bg-white text-slate-800'">
                        </div>
                        ...
                        <div class="space-y-2" x-show="category === 'Asisten Keperawatan'" x-transition>
                            ...
                            <input type="text" name="req_major_value" :disabled="majorStatus === 'nonaktif'" value="{{ old('req_major_value', $majorValue) }}" placeholder="Contoh: Keperawatan, Asisten Keperawatan" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-blue-500 transition-all" :class="majorStatus === 'nonaktif' ? 'bg-slate-50 text-slate-400 border-slate-100' : 'bg-white text-slate-800'">
                        </div>
                        ...
                        <div class="space-y-4 border border-slate-100 bg-slate-50/30 p-4 rounded-2xl" x-show="category !== 'Asisten Keperawatan' || placementChoicesStatus === 'nonaktif'" x-transition>
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-707 flex items-center gap-1.5">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <select id="location-city" name="location_city"
                                            ...
                                            :required="(placementStatus !== 'nonaktif' && placementType === 'specific') || (placementStatus === 'nonaktif' && category !== 'Cleaning Service')">
                                            <option value="{{ old('location_city', $posting->location_city) }}">
                                                {{ old('location_city', $posting->location_city) ?: 'Pilih Kota/Kabupaten' }}
                                            </option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="space-y-2" x-show="category === 'Cleaning Service' && placementStatus === 'nonaktif'" x-transition>
                            ...
                            <input type="text" name="req_placement_choices_value" :disabled="placementStatus !== 'nonaktif'" :required="category === 'Cleaning Service' && placementStatus === 'nonaktif'" value="{{ old('req_placement_choices_value', $placementChoicesValue) }}" placeholder="Pisahkan wilayah dengan koma, contoh: Cakung, Lebak Bulus" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:outline-none focus:border-blue-500 transition-all" :class="placementStatus !== 'nonaktif' ? 'bg-slate-50 text-slate-400 border-slate-100' : 'bg-white text-slate-800'">
                        </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="date" name="active_until" value="{{ old('active_until', $posting->active_until?->format('Y-m-d')) }}"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="shift_type" class="w-full mt-2 px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-blue-500 transition-all bg-white" :required="['Driver Ambulance', 'Asisten Keperawatan', 'Cleaning Service', 'Runner'].includes(category)">
                                        ...
                                        <option value="none" @selected(old('shift_type', $posting->shift_type ?? 'none') === 'none')>Tidak ada</option>
                ...
                <div class="flex justify-end gap-3 border-t border-slate-100 pt-6">
                    <button type="submit"
                        class="px-8 py-3 rounded-2xl bg-gradient-to-r from-amber-600 to-orange-600 text-white text-sm font-bold shadow-md hover:shadow-lg hover:brightness-105 active:scale-95 transition-all">
